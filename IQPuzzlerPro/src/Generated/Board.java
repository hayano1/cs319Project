...
import java.io.*;
import AllPanels.congratulationPanel;
...
import static Generated.allGamesPanelHere.pausePanel;
...
public class Board extends JPanel implements KeyListener {
	...
	//Constructor
	public Board(int width, int height, int TileTypee, String gameMode ,String level) {
		setFocusable(true);
		addKeyListener(this);
                
                pausePanel = new pausePanel(TileType);
		...
		if( TileType == 0) {
                ...
                if(gameMode.equals(Indeces.normalTileGame) || gameMode.equals(Indeces.diamondTileGame)){
                    gameSesh = new GameSession(1, TileTypee, "Normal" ,1 ,level);
                }
                else if( gameMode.equals(Indeces.savedGame)){
                    gameSesh = new GameSession(1, TileTypee, "LoadGame",1 ,level);              
                }
                else if( gameMode.equals("Generated")){
                    gameSesh = new GameSession(1, TileTypee, "Generated" ,1 ,level);
                }
                else if( gameMode.equals("Story")){
                    gameSesh = new GameSession(1, TileTypee, "Story" ,1 ,level);
                }
...
//                        }
                        ...
                        System.out.println("Escape fsndhfjkaskdf");
            ...
            if (TileType == 0) {
                    ...
                    for (int i = 0; i < boardHeight; i++) {
                            for (int j = 0; j < boardWidth; j++) {
                                    ...
                                    currWidth = j * Tile.TileWidth + boardMargin;
	...
	public void placePieceIntoBoard() {
		...
		System.out.println("PX: " + eligibleForPlacementOriginCoords.getX()  + "   PY: "  + eligibleForPlacementOriginCoords.getY()  );
                ...
                if( TileType == 0){
                    System.out.println("Entered to chagne stugfg");
                    eligibleForPlacementOriginCoords.setX(eligibleForPlacementOriginCoords.getX() );
                }
	...
	public boolean checkForEndGame(){
		...
		if (fullBoard) {
			...
			System.out.println("@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@");
                        ...
                        try{
                            File file = null;
                            FileOutputStream updatedFile = null;
                            if( gameMode.equals(Indeces.normalTileGame)){
                                file = new File(Indeces.board1LevelStatus); 
                                pathh = Indeces.board1LevelStatus;
                            }
                            else if(gameMode.equals(Indeces.diamondTileGame) ){
                                file = new File(Indeces.board2LevelStatus);
                                 pathh = Indeces.board2LevelStatus;
                            }
                            else if( gameMode.equals(Indeces.savedGame) ){
                                //file = new File("Stories/board2LevelStatus.txt");
                                //updatedFile = new FileOutputStream("Levels/board2LevelStatus.txt");
                            }
                            ...
                            while( (line = level.readLine()) != null) {
                                ...
                                System.out.println( this.level + " " +  line.substring(0, line.indexOf(" ")) + "  '''''''''''''''''''''");
                                if( line.substring(0, line.indexOf(" ")).equals( this.level )){
                                   String newUpdate = line;
                                   newUpdate = newUpdate.substring(0, newUpdate.indexOf(" ")+1);
                            ...
                            updatedFile = new FileOutputStream(pathh);
                            updatedFile.write(sb.toString().getBytes());
                            updatedFile.close();
                        ...
                        congratulationPanel congratz = new congratulationPanel( gameMode ,totalMoves);
                        cs319.main.window.add(congratz);
                        cs319.main.window.revalidate();
                        cs319.main.window.repaint();  
	...
	public void fixAllPiecesAndTiles() {
		...
		for (int i = 0; i < length; i++) {
			...
			if (!(Boolean) thisPieceInfo.get(Indeces.InBoard)) {
				...
				if( TileType == 0) {
					piecesArray.get(i).setConstructorXandY(xCord * Tile.TileWidth + boardMargin, yCord * Tile.TileHeight);
				}
				else if( TileType == 1) {
					piecesArray.get(i).setTiltedConstructor(xCord * Tile.TileWidth, yCord * Tile.TileHeight);
				}
	...
	public static String saveGame(int theTileType) {
		...
		try {
			...
			File a = new File("SavedGames//" + savedDate + "~"+ TileType +".txt");
			if (a.createNewFile())
			{
			    System.out.println("File is created!");
			} else {
			...
			writer.write( TimerPanel.getTimeLeft() + "");
	...
	class MouseListenerForBoard extends MouseAdapter implements MouseMotionListener {
		...
		public void mouseReleased(MouseEvent e) { // done coloring problems
			...
			if( TileType == 0) {
				x = (e.getX() - boardMargin) / Tile.TileWidth;
				y = e.getY() / Tile.TileHeight;
                                
                                System.out.println( x + " ~released~  " + y);
			...
			if (selectedPiece != null) {
				...
				if (x >= 0 && x < boardWidth && y >= 0 && y < boardHeight) {
					...
					if( TileType == 0) {
						...
						p = new Point(x * Tile.TileHeight + boardMargin, y * Tile.TileWidth);
					}
					else if(TileType == 1) {
		...
		public void mouseMoved(MouseEvent e) {
			...
			if(TileType == 0 ) {
				xx = (x-boardMargin) / Tile.TileWidth ;
