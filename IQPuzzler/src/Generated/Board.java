...
public class Board extends panelTemp implements KeyListener {
	...
	public Board(int width, int height, int TileTypee) {
		setFocusable(true);
		addKeyListener(this);
		addMouseListener(new MouseAdapter() { // <-------added this block
		...
		JLabel aaaa = new JLabel("SDFASDFASDFASDF~!!!!!!!!!!!!");

		// this.add(new JPanel(),BorderLayout.EAST);
		this.add(aaaa, BorderLayout.EAST);
		boardWidth = width;
		boardHeight = height;
		totalMoves = 0;
				...
				for (int j = 0; j < boardWidth; j++) {
					...
					for (int k = 0; k < 26; k++) {
						if (id + 1 == illegalTile[k]) {
							boo = false;
							// System.out.println( illegalTile[k] + " ->");
							break;
						}
					}
	...
	public void fixAllPiecesAndTiles() {
		...
		for (int i = 0; i < length; i++) {
			if (!(Boolean) gameSesh.allPieces.get(i).get(Indeces.InBoard)) {

				int xCord = (Integer) gameSesh.allPieces.get(i).get(Indeces.xInBoaard);
				int yCord = (Integer) gameSesh.allPieces.get(i).get(Indeces.yInBoaard);

				System.out.println("ASDFASDF " + xCord + "  " + yCord);
				piecesArray.get(i).setConstructorXandY(xCord * Tile.TileWidth, yCord * Tile.TileHeight);
				piecesArray.get(i).setPermaPlaced(!(Boolean) gameSesh.allPieces.get(i).get(Indeces.InBoard));
	...
	public void paintComponent(Graphics g) {
		...
		ArrayList<Piece> temp = piecesArray;// .getAllPieces();
		int length = temp.size();

		for (int i = 0; i < length; i++) {
			Piece tPiece = temp.get(i);
			tPiece.paintComponent(g);
		}
	...
	public double[] getTiltedCoordsForPiece(int xC, int yC) {
		...
		double x1 =  (xC+1)*Tile.TileWidth -w;
	...
	class MouseListenerForBoard extends MouseAdapter implements MouseMotionListener {
			...
			if (selectedPiece != null) {
				...
				if (x >= 0 && x < boardWidth && y >= 0 && y < boardHeight) {
					...
					if( TileType == 0) {
						...
						//System.out.println(e.getX() + " cor " +e.getY());
						
						//xx - temp.get(i).getY() + selectedPieceYCord
						...
						System.out.println("XXXX: " + ret[0] + "    YYYYY: " + ret[1]);
					...
					//System.out.println(x + " " + y + " |||||| " + pXCord + "  " + pYCord);

					//System.out.println(eligibleForPlacement + " <----");
		...
		public void mouseDragged(MouseEvent e) {
			// System.out.println(e.getPoint() + " asdas");
		...
		public void mouseMoved(MouseEvent e) {
			...
			if (xx >= 0 && xx < boardWidth && yy >= 0 && yy < boardHeight) { // if we are inside the grid we now can do all the operations to make the grid and the pieces work together
				...
				// System.out.println(xx + " " + yy + " " + oldTileX + " "
				// + oldTileY + " !!!!!!!!!!!!!!!!!!!!!!!!!!");
				...
				if (selectedPiece != null) {
					...
					//System.out.println(selectedPieceXCord + "  " + selectedPieceYCord + " <-");
					...
					for (int i = 0; i < length; i++) {
						...
						if( TileType == 0) {
							...
							checkX = xx - temp.get(i).getY() + selectedPieceYCord;
							checkY = yy + temp.get(i).getX() - selectedPieceXCord;
